...
- **Horizontal scaling limitation**: Channels track active conversations in instance-local memory (`self._conversations`). Works perfectly for single-instance deployments. In multi-instance deployments behind a load balancer, webhooks may route to a different instance than the one that handled the connection/message, so the instance holding the session never sees the `CONVERSATION_UPDATED`/`CLOSED` cleanup. Recommended solutions for the routing problem itself: sticky sessions (route by conversation_id) or shared state store (Redis/database). Voice call events (status/AMD/recording) are a second, independently-arriving traffic class with the same constraint — they carry only the `CallSid`, so `VoiceChannel.end_call()` resolves the session by scanning `self._conversations` and finds nothing when the event lands on an instance that never saw the call. The Twilio-side hangup still succeeds; only local session cleanup is missed.
- **Conversation sweeper**: The resulting session leak is backstopped without shared state. Each channel runs a background task that reconciles `self._conversations` against Orchestrator every `TACConfig.conversation_sweep_interval` seconds (default 300; set to `None`, or `TWILIO_CONVERSATION_SWEEP_INTERVAL` to `none`/`off`/`disabled`/`0`, to disable) and calls `_end_conversation` for anything Orchestrator reports `CLOSED` or 404s — so `on_conversation_ended` still fires, just later. Each pass checks tracked conversations one at a time via `ConversationClient.get_conversation`. The Conversations API has no ID-set filter and no batch-status endpoint (`GET /v2/Conversations` filters only on `status`, `channelId`, `addresses`), so the alternative would be paging every open conversation in the configuration and inferring closure from absence — work proportional to account-wide traffic this deployment doesn't control, and unsound besides, since a paginated listing isn't a snapshot and a conversation can shift pages mid-walk. One request per tracked conversation scales with the instance's own load and every answer is authoritative. `ACTIVE`/`INACTIVE` and any inconclusive error leave the session alone, so a transient Orchestrator failure can't tear down live conversations. The task starts lazily on the first `_start_conversation` (channels are often built at import time, before a loop exists) and is inert in ConversationRelay-only mode. Stop it explicitly with `await channel.stop_conversation_sweeper()`. This bounds leak *duration*, not dict size — it does not cap `_conversations`.
