...
def _agent_participant(
    address: str = "rcs:twilio_signal_test_agent", participant_id: str = "comms_participant_agent"
) -> ParticipantResponse:
    return ParticipantResponse(
        ...
        name="Agent",
        type="AI_AGENT",
        addresses=[ParticipantAddress(channel="RCS", address=address)],
    )
...
def _customer_participant() -> ParticipantResponse:
    return ParticipantResponse(
        ...
        name="Customer",
        type="CUSTOMER",
        addresses=[ParticipantAddress(channel="RCS", address="rcs:+12345678901")],
    )
...
async def test_inbound_agent_address_derived_from_recipient() -> None:
    """Reconcile is called with the webhook's recipient sender ID as the agent address."""
    cfg = get_test_config()
...
async def test_inbound_to_unconfigured_number_is_dropped() -> None:
    ...
    tac = TAC(get_test_config())  # only rcs:twilio_signal_test_agent
    channel = RCSChannel(tac)
    ...
    with patch.object(channel, "_reconcile_participants", new=AsyncMock()) as mock_reconcile:
        ...
        webhook["data"]["recipients"][0]["address"] = "rcs:unconfigured_agent"
        await channel.process_webhook(webhook)
